...
export default function DatePicker({
  ...
  // Form states matching reference image
  ...
  // Generate calendar grid (including days from prev/next months)
  const calendarDays = () => {
    ...
    // Next month padding to fill full 5/6 weeks
  ...
  return (
    <div ref={containerRef} data-card-popover="true" className={`relative inline-block ${className}`}>
      ...
      {/* Popover Matching Reference Design */}
      {isOpen && (
        <div
          role="dialog"
          aria-label="Dates"
          className={`absolute top-full mt-1.5 z-[70] w-[320px] max-h-[82vh] overflow-y-auto scrollbar-hide rounded-2xl bg-surface border border-line shadow-pop text-ink p-4 max-w-[calc(100vw-24px)] animate-in fade-in-50 zoom-in-95 duration-150 ${align === "right" ? "right-0" : "left-0"
            }`}
        >
          {/* Header with Title and Close Button */}
          <div className="relative flex items-center justify-center pb-3 mb-3 border-b border-line">
            <h3 className="text-sm font-semibold text-ink">Dates</h3>
            <button
              type="button"
              onClick={() => setIsOpen(false)}
              aria-label="Close dates popover"
              className="absolute right-0 top-0 w-7 h-7 rounded-lg flex items-center justify-center text-muted hover:text-ink hover:bg-surface-2 transition-colors cursor-pointer"
            >
              <X size={16} />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between px-1 mb-3">
            <div className="flex items-center gap-0.5">
              <button
                ...
                onClick={prevYear}
                title="Previous Year"
                className="w-7 h-7 rounded-md flex items-center justify-center text-muted hover:text-ink hover:bg-surface-2 transition-colors cursor-pointer"
              >
          ...
          {/* Weekdays Row */}
          <div className="grid grid-cols-7 gap-1 text-center text-[11px] font-semibold text-muted mb-1.5">
            {WEEKDAY_NAMES.map((w) => (
            ...
            {calendarDays().map((item, idx) => {
              ...
              return (
                <button
                  ...
                  className={`h-8 rounded-lg text-xs font-medium flex items-center justify-center transition-all cursor-pointer select-none ${selected
                      ? "bg-blue-600 text-white font-bold shadow-sm ring-1 ring-blue-600"
                      : item.isCurrentMonth
                        ? "text-ink hover:bg-surface-2"
                        : "text-muted/40 hover:bg-surface-2/60"
                    } ${today && !selected ? "underline font-bold text-blue-600" : ""}`}
                >
                  {item.date.getDate()}
                </button>
              );
          ...
          <div className="space-y-3 pt-2 border-t border-line text-xs">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className={`flex-1 rounded-lg border px-2.5 py-1.5 text-xs text-ink transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30 ${hasStartDate
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className={`flex-1 rounded-lg border px-2.5 py-1.5 text-xs text-ink transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30 ${hasDueDate
                      ? "bg-surface border-line"
                      : "bg-surface-2 border-line/60 text-muted/60 cursor-not-allowed"
                    }`}
                />
                <div className="relative">
                  <select
                    ...
                    className={`appearance-none rounded-lg border pl-2.5 pr-7 py-1.5 text-xs text-ink transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30 cursor-pointer ${hasDueDate
                        ? "bg-surface border-line"
                        : "bg-surface-2 border-line/60 text-muted/60 cursor-not-allowed"
                      }`}
                  >
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none rounded-lg border border-line bg-surface px-2.5 py-1.5 text-xs text-ink transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30 cursor-pointer"
                >
            ...
            <div className="pt-2 space-y-2">
              <button
                type="button"
                onClick={handleSave}
                className="w-full py-2 px-4 rounded-lg bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold text-xs sm:text-sm transition-all shadow-sm cursor-pointer"
              >
                Save
              </button>
              <button
                ...
                className="w-full py-2 px-4 rounded-lg bg-surface-2 hover:bg-surface-3 active:bg-surface-3 text-ink font-medium text-xs sm:text-sm border border-line transition-colors cursor-pointer"
              >
                Remove
              </button>
            </div>
